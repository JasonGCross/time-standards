...
- (void) setSettingName:(NSString *) value {
	if ([[value lowercaseString] isEqualToString: @"time standard"]) {
		settingName = @"Time Standard";
	}
}

+ (TimeStandardController *) timeControllerWithDefaults {
	TimeStandardController * newObject =  [[TimeStandardController alloc] initWithStyle:UITableViewStylePlain];
	if (newObject) {
		[newObject setSettingName: @"Time Standard"];
	}
	return newObject;
}
...
- (void)viewDidLoad {
	...
	// self.navigationItem.rightBarButtonItem = self.editButtonItem;
	[self setSettingName: @"Time Standard"];
	
	if (timeStandardDataAccess != nil) {
		[timeStandardDataAccess openDataBase];
		settingList = [[timeStandardDataAccess getTimeStandardNames] retain];
	}
	else {
